...
},processAlt:function(){if(this.sd.alt){var F;var C=ForeSee.$random(0,100);var E=0;var B=this.sd.alt;
...
break}}}},inviteShown:function(){if(ForeSee.$defined(ForeSee.c().fsr$get("invite"))){return true}return false
...
if(this.sd.pop.what=="survey"){this.popSurvey()}else{if(this.sd.pop.what=="qualifier"){this.popQualifier()
}else{if(this.sd.pop.what=="url"){this.popUrl()}else{if(this.sd.pop.what=="script"){this.popScript()
}}}}}else{if(this.sd.pop.when=="both"){this.popTracker();this.popSurvey()}}}},processQualifier:function(){if(!ForeSee.canceled){this.popSurvey()
}else{this.popCancel()}},popImmediate:function(){var A=this;var B=ForeSee.c();if(this.trackerRunning()){if(ForeSee.Browser.Type.name!="Firefox"||!this.sd.qualifier.dhtml){B.fsr$set("force",1)
}else{this.cancelTracker();(function(){ForeSee.log(300,ForeSee.c().fsr$get("current"));A.prepareDHTML("qualifier","qualified")
}).fsr$delay((this.sd.qualifier.delay||0)*1000)}}else{ForeSee.c().fsr$set("survey",1);if(this.sd.pop.what=="survey"){this.popSurvey()
}else{if(this.sd.pop.what=="qualifier"){this.popQualifier()}else{if(this.sd.pop.what=="url"){this.popUrl()
}else{if(this.sd.pop.what=="script"){this.popScript()}}}}}},popSurvey:function(){var B=this.sd.survey;
var A=this.sd.pop;this.popMain(this.sid(),B.width,B.height,A.pu,B.loading,"Survey")},popFeedback:function(){var C=ForeSee.$P();
var B="feedback";var A=ForeSee.locale;if(A){B=B+"-"+A}this.popMain(B,C.survey.width,C.survey.height,false,C.survey.loading,"600")
},popMain:function(C,B,I,J,D,H){var G=ForeSee.services.survey;var E=new ForeSee.Hash({sid:C,cid:ForeSee.id,version:ForeSee.version}).fsr$toQueryString();
var F=ForeSee.CPPS.fsr$toQueryString();var A=document.location.protocol+"//"+G.host+G.path+G.url+"?"+E+"&"+F;
if(D){this.page(ForeSee.$P().loading);A=ForeSee.files+ForeSee.$P().loading.url+"?url="+A}this.pop("fsr"+H,A,(window.screen.width-B)/2,(window.screen.height-I)/2,B,I,J);
...
}).fsr$delay(0)},popUrl:function(){var B=ForeSee.$P().survey.width;var A=ForeSee.$P().survey.height;
this.pop("fsrOther",this.sd.pop.url,(window.screen.width-B)/2,(window.screen.height-A)/2,B,A)},popScript:function(){new ForeSee.Asset.javascript(this.sd.pop.script)
},sid:function(D){var F=ForeSee.c();var C=!D?this.sd.name:D;var A=this.sd.pop.now;if(A){C=C+"-"+A
}var E=ForeSee.qid;if(E){C=C+"-"+E}var B=F.fsr$get("locale");if(B){C=C+"-"+B}return C},pop:function(D,C,H,G,F,A,E,B){(function(){new ForeSee.Browser.Popup(C,{name:D,toolbar:0,location:0,directories:0,status:0,scrollbars:1,resizable:1,width:F,height:A,x:H,y:G,blur:E,menubar:0})
